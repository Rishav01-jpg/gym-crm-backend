...
try {
 const tcRes = await axios.get("/api/training-classes");
  ...
  console.log("Training classes not loaded");
  setTrainingClasses([]);
}
...
trainingClass: newType === "trainingClass" ? attendance.trainingClass : "",
...
if (
  attendance.attendanceType === "trainingClass" &&
  !attendance.trainingClass
) {
  newErrors.trainingClass = "Training class is required";
}
...
// ✅ ADD THIS FOR TRAINING CLASS
if (
  attendance.attendanceType === "trainingClass" &&
  attendance.trainingClass
) {
  attendanceData.trainingClass = attendance.trainingClass;
}
                ...
                <Grid item xs={12} md={6}>
                  ...
                  >
                    ...
                    >
                      ...
                      <MenuItem value="trainingClass">Training Class</MenuItem>
...
{attendance.attendanceType === "trainingClass" && (
  <Grid item xs={12} md={6}>
    <FormControl
      fullWidth
      required
      error={!!errors.trainingClass}
    >
      <InputLabel id="training-class-label">
        Training Class
      </InputLabel>

      <Select
        labelId="training-class-label"
        name="trainingClass"
        value={attendance.trainingClass}
        onChange={handleChange}
      ...
      {errors.trainingClass && (
        <FormHelperText>{errors.trainingClass}</FormHelperText>
      )}
    </FormControl>
  </Grid>
)}
